...
这个项目面向企业私有知识库场景，目标不是搭一个简单 Demo，而是把私有文档从解析、入库、检索、精读、引用回答、离线评测到 Agent 编排逐步打通。当前主线已经切到新的企业知识 RAG 架构，并在此基础上接入 LangGraph 驱动的 Agent 编排层。
...
旧 `Chunk / Segment / mode mix/local/global` 不是主线。`rag/agent/**` 是新的 Agent 编排层，按 `docs/superpowers/specs/2026-05-07-agent-architecture-design.md` 作为唯一权威推进，不兼容旧 `AnalysisAgentService / AgentRunState` 等旧接口。
...
### 11. Agent 编排层遵循 Tool-Centric + LangGraph

Agent 层不再保留旧的 run-state 服务接口，而是把 Agent 定义、工具契约、运行配置和 LangGraph 状态拆开：

- `ToolSpec / ToolRegistry` 描述工具边界，工具未注册、参数非法、执行失败、预算不足都返回结构化失败结果。
- `AgentRunConfig / RuntimeRegistry / BudgetLedger` 分离可序列化配置和不可序列化运行时对象，避免把 lock、event、预算 ledger 放进 checkpoint state。
- `AgentState` 使用 TypedDict 和 reducer，显式合并 evidence、citations、tool results、subtask results 和并行子任务状态。
- `TaskDAG + Send()` 支持 Orchestrator 将复杂任务拆成可并行的子 Agent，并在失败时 fail closed。
- Phase A memory 只做 working memory compaction / injection；长期 memory 放到后续阶段，RAG evidence 的权威性高于 memory。
...
Agent 层当前处在 `agent/design-implementation` 分支，重点已经从概念设计推进到可测试的工程骨架。实现原则是：以 `docs/superpowers/specs/2026-05-07-agent-architecture-design.md` 为唯一权威，不兼容旧 agent API，不为了迁就旧 `AnalysisAgentService / AgentRunState` 保留适配层。

已完成的部分：

- 契约层：`ToolSpec`、`ToolPermissions`、`ToolResult`、`ToolError`、`AgentRunConfig`、`RuntimeRegistry`、`BudgetLedger`、`AgentDefinition`、`ModelPolicy`、`ToolPolicy`、`AgentState`。
- 工具运行层：`ToolRegistry` 支持 callable runner，输入/输出 Pydantic 校验，未注册、未实现、参数非法、输出非法、预算不足、timeout、retry 都会形成结构化结果。
- Agent 注册与组合：`AgentRegistry`、`AgentToolSpec`、`AgentAsToolRunner` 已落地，子 Agent 使用 `derive_child_config()` 继承父 run 的权限、source scope、执行位置偏好和递归深度约束。
- LangGraph 基础图：`route -> plan -> execute -> observe -> evaluate -> synthesize` 主骨架已实现，支持 `execute_subagent` 的 `Send()` 并行扇出和回收。
- TaskDAG 调度：`SubTaskNode / TaskEdge / TaskDAG / SubTaskResult` 已实现唯一 ID 校验、依赖校验、环检测、ready subtask 选择、预算预留、失败子任务阻断下游。
- 失败语义：普通 tool 和 subagent 都 fail closed；子 Agent 返回 failed、runner 抛异常、runtime handles 缺失、plan provider 缺失都会写入明确失败状态。
- 上下文与短期记忆：Phase A working memory compaction / context injection 已接入 evaluate 决策路径；长期 memory 未进入当前阶段。
- 内置能力：`ResearchAgent` service factory、RAG tool specs、LLM tool specs、builtin tool registry 已提供，默认不伪造 runner。

仍在推进的部分：

- 真实 LLM plan/evaluate provider：当前图已经支持注入结构化 provider，但生产默认 provider 和 prompt/eval 还需要继续建设。
- pause / resume：当前状态机可以进入 `paused`，真正基于 LangGraph `interrupt()` + checkpointer + `Command(resume=...)` 的恢复链路还未完整落地。
- Orchestrator 完整产品化：TaskDAG 调度和 subagent runner 已具备，但 compare/factcheck/synthesize 等更多内置 AgentDefinition 还需要继续补齐。
- RAG tool runner 与主 RAG runtime 的深度绑定：工具契约已经有了，后续要把 vector_search、keyword_search、grounding、rerank 等 runner 接到当前 L3-L6 主链路。
- Agent CLI/API 与 golden eval：契约层和 graph 层已具备测试，后续需要用 golden case / eval 脚本评估真实规划质量、回答质量和多 Agent 协作效果。
...
│   ├── memory/                        # Phase A working memory compaction / context injection
│   ├── tools/                         # ToolSpec / ToolRegistry / RAG & LLM tool specs
│   ├── builtin/                       # 内置 ResearchAgent 定义与 service factory
