...
class SQLiteGraphRepo:
    ...
    def _migrate_legacy_nodes(self) -> None:
        columns = {row["name"] for row in self._conn.execute("PRAGMA table_info(nodes)").fetchall()}
    ...
    def _migrate_legacy_edges(self, table_name: str) -> None:
        columns = {row["name"] for row in self._conn.execute(f"PRAGMA table_info({table_name})").fetchall()}
    ...
    def delete_node(self, node_id: str) -> None:
        self._conn.execute("DELETE FROM node_evidence WHERE node_id = ?", (node_id,))
        self._conn.execute("DELETE FROM node_aliases WHERE node_id = ?", (node_id,))
        for table_name in ("edges", "candidate_edges"):
            edge_rows = self._conn.execute(
                f"SELECT edge_id FROM {table_name} WHERE from_node_id = ? OR to_node_id = ?",
                (node_id, node_id),
            ).fetchall()
    ...
    def delete_edge(self, edge_id: str, *, include_candidates: bool = True) -> None:
        ...
        for table_name in table_names:
            self._conn.execute(f"DELETE FROM {table_name} WHERE edge_id = ?", (edge_id,))
        self._conn.commit()
