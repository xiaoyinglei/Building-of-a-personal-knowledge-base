        ...
        *,
        ...
        for index, item in enumerate(evidence_pack, start=1):
            ...
            chunk_type = item.chunk_type or item.special_chunk_type or (
                item.chunk_role.value if item.chunk_role is not None else "child"
            )
            lines.extend(
                [
    ...
    def _payload_sections(
        ...
        for item in raw_sections:
            ...
            if not isinstance(title, str) or not isinstance(text, str) or not text.strip():
                continue
            resolved_ids = [value for value in evidence_ids if isinstance(value, str)] if isinstance(
                evidence_ids, list
            ) else []
    ...
    def _materialize_answer(
        ...
        *,
        ...
        overall_grounded = grounded and (
            self._section_grounded(answer_text, evidence_pack)
            or len(answer_sections) > 1
        )
