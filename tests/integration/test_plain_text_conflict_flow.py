...
def test_cli_ingest_persists_explicit_access_policy_into_metadata(tmp_path: Path) -> None:
    container = build_test_container(tmp_path)
    clear_container_factory()
    set_container_factory(lambda: container)
    ...
    source = container.metadata_repo.list_sources()[0]
    assert source.effective_access_policy.residency is Residency.LOCAL_REQUIRED
    assert source.effective_access_policy.external_retrieval is ExternalRetrievalPolicy.DENY
    assert source.effective_access_policy.allowed_runtimes == frozenset({RuntimeMode.FAST})
    assert source.effective_access_policy.allowed_locations == frozenset({ExecutionLocation.LOCAL})
    assert source.effective_access_policy.sensitivity_tags == frozenset({"private"})
